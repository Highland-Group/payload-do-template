...
This is a "Proof of Concept" for deploting to Digital Ocean using the official [Payload Website Template](https://github.com/payloadcms/payload/blob/3.x/templates/website). 
...
1. Fill in the `REPLACE_ME` secrets (`PAYLOAD_SECRET`, `CRON_SECRET`, `PREVIEW_SECRET`) with real generated values in a local, uncommitted copy — never commit real secret values into `.do/app.yaml`.
